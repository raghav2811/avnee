<x-admin-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create Home Experience Section') }}
        </h2>
    </x-slot>
            ...
            <form action="{{ route('admin.home-sections.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                            <h3 class="text-lg font-medium mb-4 text-gray-900 dark:text-gray-100 border-b pb-2">Technical Info</h3>
                            <div>
                                ...
                                <select name="section_id" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 rounded-md shadow-sm" required>
                                    ...
                                    <option value="shop_by_style" {{ old('section_id') == 'shop_by_style' ? 'selected' : '' }}>Shop By Style (Square Grid)</option>
                                    <option value="just_in" {{ old('section_id') == 'just_in' ? 'selected' : '' }}>Just In (Banner Experience)</option>
                                    <option value="shop_by_price" {{ old('section_id') == 'shop_by_price' ? 'selected' : '' }}>Shop By Price (Round Badges)</option>
                                    <option value="best_buys" {{ old('section_id') == 'best_buys' ? 'selected' : '' }}>Best Buys (Product Slider)</option>
                                    <option value="shop_the_look" {{ old('section_id') == 'shop_the_look' ? 'selected' : '' }}>Shop The Look (Video/Image Grid)</option>
                                    <option value="top_collections" {{ old('section_id') == 'top_collections' ? 'selected' : '' }}>Top Collections (Featured Grid)</option>
                                    <option value="popular_pieces" {{ old('section_id') == 'popular_pieces' ? 'selected' : '' }}>Bestselling Styles (Simple Slider)</option>
                                </select>
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="subtitle" value="{{ old('subtitle') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="limit" value="{{ old('limit', 8) }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                <div class="flex items-center pt-2">
                                    <input type="checkbox" name="is_active" id="is_active" checked class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500 cursor-pointer">
                        ...
                        <div class="bg-white dark:bg-gray-800 p-6 shadow-sm sm:rounded-lg">
                            <h3 class="text-lg font-medium mb-4 text-gray-900 dark:text-gray-100 border-b pb-2">Media</h3>
                            <div class="space-y-4">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Redirect URL</label>
                                    <input type="text" name="redirect_url" value="{{ old('redirect_url') }}" class="mt-1 block w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                </div>
                                <div>
                        ...
                        <div class="mt-6 flex flex-col gap-3">
                            <button type="submit" class="w-full px-6 py-3 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 shadow-lg transform transition active:scale-95">Create Section</button>
                            <a href="{{ route('admin.home-sections.index') }}" class="w-full px-6 py-3 text-center text-sm text-gray-700 dark:text-gray-300 border rounded-lg hover:bg-gray-50 font-medium tracking-wide">Cancel</a>
                        </div>
                    </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg flex flex-col h-full overflow-hidden">
                            ...
                            <div id="product-grid" class="flex-1 min-h-[500px] max-h-[800px] overflow-y-auto p-6 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 bg-gray-50/30 dark:bg-gray-900/10">
                                @foreach($products as $product)
                                    <label class="product-item hidden relative flex items-start p-4 cursor-pointer rounded-xl border-2 bg-white dark:bg-gray-800 border-transparent transition-all" 
    ...
    <script>
        function filterProductsByBrand(brandId) {
            ...
            products.forEach(p => {
                if(p.getAttribute('data-brand') == brandId) {
                    p.classList.remove('hidden');
                } else {
                    p.classList.add('hidden');
                    // Uncheck if it was checked but now hidden
                    const cb = p.querySelector('input[type="checkbox"]');
                    if(cb.checked) {
                        cb.checked = false;
                        toggleProductStyle(cb);
                    }
                }
            });
        }
        ...
        // Run filter on load if old value exists
        @if(old('brand_id'))
            filterProductsByBrand('{{ old('brand_id') }}');
        @endif
    </script>
</x-admin-layout>
