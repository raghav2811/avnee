    ...
    <div class="px-6 py-8 sm:px-10">
        <div class="max-w-4xl mx-auto">
            <div class="flex items-center gap-4 mb-10">
                ...
                <div>
                    <h1 class="text-3xl font-heading font-normal tracking-tight bg-gradient-to-r from-white to-gray-400 bg-clip-text text-transparent">Edit Feedback</h1>
                    <p class="text-sm text-gray-500 mt-1 uppercase tracking-widest text-[10px]">Modify existing customer review data</p>
                </div>
            </div>
            ...
            <div class="glass-card rounded-2xl p-8 sm:p-10 border border-white/5 shadow-2xl relative overflow-hidden">
                ...
                <form action="{{ route('admin.reviews.update', $review) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-8" x-data="{ imagePreview: '{{ $review->image ? asset('storage/'.$review->image) : null }}' }">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <!-- Brand Selection -->
                        <div class="space-y-2">
                            <label class="block text-[10px] uppercase font-bold text-gray-400 tracking-[0.2em]">Storefront Brand</label>
                            <select name="brand_id" required class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#b87333]/50 focus:ring-0 transition-all text-sm appearance-none">
                                ...
                                <option value="{{ $brand->id }}" {{ $brand->id == $review->brand_id ? 'selected' : '' }}>{{ $brand->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <!-- Product Selection -->
                        <div class="space-y-2">
                            <label class="block text-[10px] uppercase font-bold text-gray-400 tracking-[0.2em]">Target Product</label>
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="user_name" value="{{ old('user_name', $review->user_name) }}" required class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#b87333]/50 focus:ring-0 transition-all text-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="location" value="{{ old('location', $review->location) }}" placeholder="e.g. Hyderabad, Telangana" class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#b87333]/50 focus:ring-0 transition-all text-sm">
                            @error('location') <p class="text-red-500 text-[10px] mt-1">{{ $message }}</p> @enderror
                        </div>

                        <!-- Moderation Status -->
                        <div class="space-y-2">
                            <label class="block text-[10px] uppercase font-bold text-gray-400 tracking-[0.2em]">Moderation Status</label>
                            <select name="status" required class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#b87333]/50 focus:ring-0 transition-all text-sm appearance-none">
                            ...
                            <div class="flex items-start gap-6">
                                ...
                                <div class="flex-1 space-y-3">
                                    ...
                                    <div class="flex flex-wrap gap-3">
                                        ...
                                        @if($review->image)
                                        <label class="inline-flex items-center gap-2 cursor-pointer bg-red-950/20 border border-red-500/20 px-4 py-2 rounded-xl">
                                            <input type="checkbox" name="remove_image" value="1" class="rounded border-white/10 bg-black/40 text-red-600 focus:ring-0">
                                            <span class="text-[10px] font-bold text-red-500 uppercase tracking-widest">Remove Current</span>
                                        </label>
                                        @endif
                                    </div>
                                    <p class="text-[10px] text-gray-500 uppercase tracking-widest leading-relaxed">Ideal for customer stories. Max 2MB.</p>
                                </div>
                            </div>
                        ...
                        <div class="space-y-2 md:col-span-2">
                            <label class="block text-[10px] uppercase font-bold text-gray-400 tracking-[0.2em]">Experience Rating</label>
                            <div class="flex gap-4">
                                @for($i=1;$i<=5;$i++)
                                <label class="cursor-pointer group flex-1">
                                    <input type="radio" name="rating" value="{{ $i }}" class="peer hidden" {{ (old('rating', $review->rating) == $i) ? 'checked' : '' }}>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="comment" rows="6" required class="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white focus:border-[#b87333]/50 focus:ring-0 transition-all text-sm resize-none">{{ old('comment', $review->comment) }}</textarea>
